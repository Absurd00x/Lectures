...
// 4. Рекурсия (зависимость функции от самой себя)
...
// "void" нужен в качестве "заглушки".
...
// Переменные можно объявлять за функциями. Такие переменные
// называются глобальными. В промышленном программировании так не принято
// делать, но у нас олимпиадное, а не промышленное, так что можно:
int counter = 0;
void increment() {
  ++counter;
}

// Асимптотика, сложность, "О большое" - верхняя граница чего-то.
// Формально, это функция O(), которая принимает функцию f(n) как аргумент и
// возвращает другую функцию g(n) такую, что если её домножить на константу
// и прибавить константу, то она будет больше, чем исходная функция f(n)
// для любого входного числа. Запись на математическом языке:
...
// 3. Убираешь все константы. (n^2/2 = n^2,
//    n^2 = n^2, 2 - не константа, а форма записи)
// 4. Выбираешь самое большое слагаемое; (n^2 > n)
// O(n*(n-1)/2) = O(n*n/2 - n/2) = O(n*n - n) = O(n*n) = O(n^2)

// f(n) = n*(n-1)/2
// O(f(n)) = g(n) = n^2
// э(C0, C1): C1 * g(n) + C0 > f(n)
// 1000 * n^2 + 1000 > n*(n-1)/2

// Функция g(n) = n^2 с константами ограничивает сверху функцию f(n)

// Асимптотика нужна, чтобы ты заранее мог понять уложится ли
// твоё решение в ограничение по времени и памяти.
...
// O(n^2), n = 2*10^5;
// (2*10^5)^2 = 4*10^10 > 3*10^7
// O(n^2), n = 10^3
// (10^3)^2 = 10^6 < 3*10^7

// Чтобы научиться считать асимптотику рассмотрим как она считается на
...
vector<int> merge(vector<int> &arr, int start, int finish) {
  ...
  // log_5(2) - это константа, так что от неё можно избавиться. Это значит,
