...
int main() {
  ...
  // Метод .count() обойдёт всё дерево в худшем случае,
  // когда все элементы равны. В этом случае обход займёт O(n)
  ...
  // Но в сете/мапе/мультисете это займёт O(n) итераций, потому что
  // нужно будет обходить красно-чёрное дерево.
  // При этом прибавление единицы будет работать за O(log(n)), но
  // прибавление числа работает за O(расстояние).
  ...
  // каждый элемент справа должен сдвинуться на единицу вправа, чтобы
  ...
  // Удивительно, но это займёт O(n) времени и памяти.
  // Давайте докажем.
