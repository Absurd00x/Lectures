...
signed main() {
  ...
  // ps[i][j] = сумма первых i строк и j столбцов матрицы
  ...
  // функции с подходящим нам значением. Для этого делим
  ...
  // Но обычно пишут так:
  while (r - l > 1) {
    // Считаем середину. Снова связь с сортировкой слиянием.
    int mid = l + (r - l) / 2;
  ...
  // массива положительные. (В задаче из набора они могут быть отрицательными)
  // Наш отрезок представляется двумя границами - левой и правой.
  // Мы можем каждую из них сдвинуть.
  // Если двигаем правую, то сумма увеличивается. Отрезок увеличивается
  // Если двигаем левую, то сумма уменьшается. Отрезок уменьшается.
  // Если сумма текущего меньше x, то нужно его расширять, чтобы
  // увеличить сумму.
