...
signed main() {
  ...
  // Чтобы посчитать префикс-сумму i, нужно взять предыдущую
  // ps[i - 1] и добавить один элемент arr[i - 1]
  // ps[i] = ps[i - 1] + arr[i - 1], но чтобы было минимум +-1 пиши так:
  int n = (int)arr.size();
  vector<int> ps(n + 1);
  for (int i = 0; i < n; ++i) {
    ps[i + 1] = ps[i] + arr[i];
  ...
  // Но обычно пишут так:
  while (r - l > 1) {
    // Считаем середину. Та самая связь с сортировкой слиянием.
    int mid = l + (r - l) / 2;
